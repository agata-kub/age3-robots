...
public class IOService {
	...
	public static RosonBuilding importFromJson(File file) throws IOException {
		...
		gatesNode.forEach(gateNode -> {
			...
			// It is not the same probability we use to represent intruder!
			// if (gateNode.has("probability"))
			// gate.setProbability(gateNode.get("probability").asDouble());
		...
		robotsNode.forEach(robot -> {
			...
			Space space = spaces.values().stream().filter(s -> Objects.equals(s.getSpaceId(), spaceId))
					.collect(Collectors.toList()).get(0);
			space.isRobotThere(true);
			nodes.get(space.getNodeId()).getIncidentNodes().stream().filter(node -> gates.containsKey(node.getNodeId()))
					.forEach(gate -> gate.isRobotThere(true));

		});

		intrudersNode.forEach(intruder -> {
			String spaceId = intruder.get("spaceId").asText();
			Space space = spaces.values().stream().filter(val -> Objects.equals(val.getSpaceId(), spaceId))
					.collect(Collectors.toList()).get(0);
	...
	private static RosonBuilding createBuilding(Map<String, Node> nodes, Map<String, Gate> gates,
		...
		if (!toDelete.isEmpty())
			toDelete.forEach(gate -> building.deleteNode(gate.getNodeId()));
		return building;
	}
